...
exports.collect = async (collectible, job) => {
  ...
  const authOpts = { url: 'https://login.microsoftonline.com/${tenantId}/oauth2/token', method: RestVerb.POST, params: authParams, exprArgs };
  const authToken = await (await httpSearch(authOpts, job.logger())).extractResult('access_token');
  // Collect
  const params = getCollectParams();
  const searchOpts = { url: collectible.source, params, method: RestVerb.GET, headers: { Authorization: wrapExpr(`Bearer ${authToken}`) } };
